...
trace_PbyP = [rawdata{1} rawdata{2} rawdata{3}]';
trace_PbyP(4,2:end) = trace_PbyP(2,2:end) - trace_PbyP(2,1:end-1);
trace_PbyP([3 4],:) = trace_PbyP([4 3],:);
% trace_PbyP = [micro time(ns);
%               macro time(s);
%               Interval time between adjacent photons(s);
%               if the photon end of trace(0 or 1)];

%%
N = size(trace_PbyP,2);                   % total photon counts
M = sum(trace_PbyP(4,:));                 % total counts of burst/trace connected
...
u_lamda = ones(1,2);
Gamma = ones(1,size(trace_PbyP,2));
W_lamda =P_W_lamda_VBEM(Gamma,u_lamda,trace_PbyP,IRF);  % used for q(lamda)
[B_mat,lamda_1,log_Norm_qlamda_temp,q_lamda] = B_mat_VBEM_FLIM(trace_PbyP,W_lamda,Gamma,IRF,maxTau,resolution,1);
...
for staNum = staNum_lase(staNum_min:staNum_max)
    staNum1 = staNum+1;        % noise state as one state
    ...
    u_Pi = ones(1,staNum1);  u_zeta = ones(staNum1,staNum1);  u_lamda = ones(staNum,2); eta = maxTau;
    
    % q(Z) is defined by  Pi*, k*, zeta*(ζ),lamda*(λ), which are initialized below
    k_0 = zeros(1,staNum1)+100;     k_0(end) = 100000;
    zeta_0 = zeros(staNum1,staNum1);
    for i = 1:staNum1
        zeta_0(i,:) = zeros(1,staNum1)+1/(staNum1-1);
    end
    Pi_0    = zeros(1,staNum1)+1/staNum1; 
    ...
    A_lase      =  zeros(staNum1,staNum1,N-1);                    % Transition probability matrix of adjacent photons;
    ...
    for iter = 1:maxIter
        ...
        for k = 1:staNum1
            temp = exp(-k_0(k)*trace_PbyP(3,2:end)');
            A_lase_temp(:,:,k) = (1-temp).*zeta_0(k,:);
            A_lase_temp(:,k,k) = temp;
        end
        A_lase = permute(A_lase_temp,[3,2,1]);
        for m = 1:M-1
            A_lase(:,:,end_pos(m)) = Pi_0.*ones(staNum1,1);
        end
        
        % B_lase(i,j) ≡ p(t_j |z_j = i), 
        B_lase = [ B_mat(:,floor(trace_PbyP(1,:)/resolution)+1);...
                   zeros(1,N)+1/eta ];
        ...
        Xi        = P_Xi_VBEM(trace_PbyP,A_lase,B_lase,Alpha,Beta);    % equation S26 in SI, ω_ij^n=∑_z q(z)δ(z_n=i,z_(n+1)=j)
        
        
        [W_k,V_k] = P_WV_k_VBEM_connect(trace_PbyP,Xi);                       % used for q(k), see equations S23 & S24
        W_lamda   = P_W_lamda_VBEM(Gamma(1:end-1,:),u_lamda,trace_PbyP,IRF);  % used for q(λ), 
        ...
        [k_1,zeta_1,Pi_1,log_Norm_qk,q_k] = Para_star_VBEM_FLIM_1(trace_PbyP,W_k,V_k,W_zeta,W_Pi);
        % W_lambda → lambda*
        [B_mat,lamda_1,log_Norm_qlamda_temp,q_lamda] = B_mat_VBEM_FLIM_1(trace_PbyP,W_lamda,Gamma(1:end-1,:),IRF,maxTau,resolution,lamda_0);
...
figure
plot(trace_PbyP(2,:),seq_best,'-*')
hold on
plot(trace_PbyP(2,:),trace_PbyP(4,:)*10)
...
%% Delete the recognized noise photons
% data saved in trace_PbyP_deNoise (same format with trace_PbyP)

temp = trace_PbyP(4,:);
% noise_pos = find(seq_best == staNum_best+1);
for i = 1:length(end_pos)
    temp(start_pos(i):end_pos(i)) = i;
end
temp = temp(:,seq_best~=staNum_best+1);
temp = [temp(2:end) - temp(1:end-1) 1];


trace_PbyP_deNoise = trace_PbyP(1:3,seq_best~=staNum_best+1);
trace_PbyP_deNoise(3,:) = temp;


fid_1=fopen([path_name  file_name(1:end-4) '_deNoise.txt'], 'w');
fprintf(fid_1,'%f %f %f\n',trace_PbyP_deNoise);
fclose(fid);
